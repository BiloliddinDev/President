...
export async function ProductDetailService(productId: number) {
    ...
    return await fetcher(`/api/v1/product/by_id_and_locale?productId=${productId}&language=${lang.toUpperCase()}&countryCode=${countryCode}`);
}
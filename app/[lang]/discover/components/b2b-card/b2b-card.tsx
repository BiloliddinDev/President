...
export const B2bCard = ({className,title,desc,image}: { className?: string,title:string,desc:string, image:StaticImageData }) => {
    ...
    console.log(processedImage, 'This is processed image')
    return (
            ...
            <div>
                ...
                <div className="mt-6 flex items-center  gap-4 p-4 bg-white rounded border">
                    ...
                    <div className={'w-full'}>
                        <p className="text-sm text-primary mb-2">Please upload your logo, size should be less
                            than <br/> 3MB.</p>
                        <input id="image" type="file" accept="image/png"   className="hidden" onChange={handleImageChange}/>
                        <div className="w-full flex gap-2 items-center justify-between ">
                            <div>{uploading ? "Uploading..." : selectedImage ? selectedImage : "No file chosen"}</div>
                            <label
                                htmlFor="image"
                                className={`${buttonVariants({variant: "secondary"})} w-[30px] inline cursor-pointer`}
                            >
                                Choose File
                            </label>
                        </div>
                        ...
                        {[
                            ...
                            <div key={color} className="flex items-center">
                                <RadioGroupItem
                                    value={color}
                                    id={color}
                                    className={`w-6 h-6 rounded-full !border-1 border-transparent cursor-pointer
                                    ${bg}

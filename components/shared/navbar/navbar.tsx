...
export const Navbar = ({lang}: { lang: string }) => {
    ...
    const hoverTextColorClass = isHome && !scrolled ? "!hover:text-zinc-300" : "!hover:text-zinc-600";

    return (
        <Headroom>
            <nav
                className={`fixed w-full p-5 z-50 transition-colors duration-300 ${navClasses}`}
            >
                <div className="container hidden md:flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <NavbarModal
                            side="right"
                            title={
                                <p className={`${textColorClass} ${hoverTextColorClass} text-lg font-normal  transition-colors duration-200`}>
                                    {`${lang.toUpperCase()}($)`}
                                </p>
                            }
                        >

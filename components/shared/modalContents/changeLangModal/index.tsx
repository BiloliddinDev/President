...
const ChangeLangModal = ({
  ...
  const [currencyCookie, setCurrencyCookie] = useState<{ code: string } | null>(
    null
  );
  ...
  return (
    <div className="flex flex-col gap-4 mx-2 h-full text-primary text-sm font-medium leading-normal">
      ...
      <div>
        <h2
          onClick={handleOpen}
          className={`mb-5 ${isMobile && "flex items-center justify-between"}`}
        >
          ...
          <ul className="flex flex-col w-full gap-7">
            {currency?.map((element) => (
              ...
              >
                ...
                <span>
                  {currencyCookie?.code === element.code && <Check />}
                </span>
              </li>
            ))}
          </ul>
